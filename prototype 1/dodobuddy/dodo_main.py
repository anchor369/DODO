...
if __name__ == "__main__":
    # Buddy Integration
    try:
        # Import and initialize buddy system
        from dodo_buddy import DodoBuddy
        
        # Create a global buddy instance
        buddy = DodoBuddy(gemini_model, speak)
        
        # Enhance process_single_command
        original_process_single_command = process_single_command
        
        def enhanced_process_single_command(command):
            """Enhanced command processing with buddy features"""
            if "help" in command.lower() or command.lower().startswith(("what", "who", "how", "when", "where", "why")) or "?" in command:
                # Likely a conversation or question - use buddy
                response = buddy.process_user_input(command)
                speak(response)
                return True
            else:
                # Likely a command - use original command processing
                return original_process_single_command(command)
        
        # Replace the original function
        process_single_command = enhanced_process_single_command
        
        print("Buddy integration complete. Assistant will now support conversation.")
    except ImportError as e:
        print(f"Buddy system not available: {e}. Running with standard capabilities.")
