...
def spotify_play_pause():
    try:
        ...
        logger.error(f"Failed to toggle play/pause on Spotify: {e}")
        speak("Couldn't toggle play/pause on Spotify.")
...
def spotify_next():
    try:
        ...
        logger.error(f"Failed to play next track on Spotify: {e}")
        speak("Couldn't skip to the next track on Spotify.")
...
def spotify_previous():
    try:
        ...
        logger.error(f"Failed to play previous track on Spotify: {e}")
        speak("Couldn't go back to the previous track on Spotify.")
...
def media_play_pause():
    try:
        ...
        speak("Couldn't toggle play or pause.")
    ...
    while True:
        try:
            ...
            if command:
                ...
                elif "maximise" in command or "minimize" in command or "restore" in command:
                ...
                elif any(word in command for word in ["play", "pause", "next", "previous", "spotify"]):
                ...
                elif any(word in command for word in ["play", "pause", "next", "previous", "media"]):
                    if "play" in command or "pause" in command:
                        media_play_pause()
                    elif "next" in command:
                        media_next()
